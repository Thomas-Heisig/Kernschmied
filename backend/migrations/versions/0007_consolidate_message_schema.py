...
def _detect_orphans(conn: Connection) -> dict[str, int]:
    res: dict[str, int] = {}
    try:
        r: Result[Any] = conn.execute(sa.text("SELECT COUNT(1) FROM messages WHERE conversation_id NOT IN (SELECT id FROM chats)"))
        res['messages_without_chat'] = int(r.scalar() or 0)
    except Exception:
        res['messages_without_chat'] = 0
    try:
        r: Result[Any] = conn.execute(sa.text("SELECT COUNT(1) FROM chats WHERE node_id NOT IN (SELECT id FROM hierarchy_nodes)"))
        res['chats_without_node'] = int(r.scalar() or 0)
    except Exception:
...
def upgrade() -> None:
    ...
    if orphans.get('messages_without_chat', 0) > 0 or orphans.get('chats_without_node', 0) > 0:
