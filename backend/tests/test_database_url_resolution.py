...
def test_resolve_relative_variants_equal() -> None:
    ...
    expected = f"sqlite+aiosqlite:///{(backend_dir / 'data' / 'chat.db').resolve().as_posix()}"
...
def test_resolve_absolute_and_memory_and_postgres_unchanged() -> None:
    ...
    assert settings.resolve_database_url(abs_path, backend_directory=backend_dir) == abs_path

    memory = "sqlite+aiosqlite:///:memory:"
    assert settings.resolve_database_url(memory, backend_directory=backend_dir) == memory

    pg = "postgresql+asyncpg://user:pass@localhost/db"
    assert settings.resolve_database_url(pg, backend_directory=backend_dir) == pg


def test_resolve_is_cwd_independent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    backend_dir = settings.BACKEND_DIRECTORY
