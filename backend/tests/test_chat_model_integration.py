...
class RecordingModelService(ModelService):
    ...
    async def stream(
        ...
        *,
        ...
        yield StreamEvent.create(type=StreamEventType.MESSAGE, content=self.first_response)
...
async def test_second_request_contains_previous_conversation_history(session_factory: async_sessionmaker[AsyncSession]) -> None:
    ...
    conv_id = None
    for e in events:
        if getattr(e, "event", None) is not None and getattr(e, "event").name == "START":
            conv_id = e.conversation_id
            break
    assert conv_id is not None

    # second user message uses same conversation id
    req2 = ChatRequest(message="Wer bin ich?", conversation_id=conv_id, history=(), system_prompt=None)
