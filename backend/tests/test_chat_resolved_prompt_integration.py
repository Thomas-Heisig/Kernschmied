...
from app.services.chat_service import ChatService, ChatServiceContext, ChatRequest
from app.models.service import ModelAccessContext, ModelService
from app.contracts.model_backend import MessageRole, GenerationRequest, StreamEvent, StreamEventType
...
def _build_hierarchy(session: AsyncSession) -> None:
    nodes = [
        HierarchyNodeModel(id="system-root", type="system", name="System Root", parent_id=None, system_prompt="Befolge die zentralen Sicherheits- und Systemregeln.", prompt_priority=-1000, prompt_mode="append", is_active=True),
        HierarchyNodeModel(id="user-thomas", type="user", name="Thomas Heisig", parent_id="system-root", system_prompt="Der angemeldete Benutzer ist Thomas Heisig.", prompt_priority=-100, prompt_mode="append", is_active=True),
        HierarchyNodeModel(id="workspace-heisig", type="workspace", name="Heisig Naturstein", parent_id="user-thomas", system_prompt="Du arbeitest für Heisig Naturstein.", prompt_priority=0, prompt_mode="append", is_active=True),
        HierarchyNodeModel(id="project-angebote", type="project", name="Angebote", parent_id="workspace-heisig", system_prompt="Erstelle Angebote nach den Unternehmensregeln.", prompt_priority=10, prompt_mode="append", is_active=True),
        HierarchyNodeModel(id="chat-mueller", type="chat", name="Angebot Müller", parent_id="project-angebote", system_prompt="Der aktuelle Vorgang betrifft den Kunden Müller.", prompt_priority=20, prompt_mode="append", is_active=True),
    ]
...
async def _run_in_memory_test():
    ...
    service = ChatService(model_service=model, default_model_id="m", repository=chat_repo, history_provider=history_provider, hierarchy_session_factory=session_factory)
    ...
    req = ChatRequest(message="Wer bist du?", conversation_id=None, hierarchy_node_id="chat-mueller")
