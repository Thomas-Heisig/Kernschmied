...
class RecordingPromptResolver:
    def __init__(
        self, *, permission_service: HierarchyPermissionService | None = None
    ) -> None:
    ...
    # Class attribute exposed for tests when the resolver instance is created.
    last_instance: ClassVar[_TAny] = None
...
def test_exact_hierarchy_actor_instance_is_preserved():
    ...
    async def _inner():
        ...
        service_context = ChatServiceContext(
            request_id="r2", access=ModelAccessContext(), hierarchy_actor=actor
        )
...
def test_unauthorized_actor_prevents_model_invocation():
    ...
    async def _inner():
        ...
        service_context = ChatServiceContext(
            request_id="r3", access=ModelAccessContext(), hierarchy_actor=actor
        )
        ...
        try:
            ...
            assert (
                "Leseberechtigung" in str(raised)
                or "Leseberechtigung" in raised.message
            )
...
def test_missing_hierarchy_actor_is_rejected():
    ...
    async def _inner():
        ...
        service_context = ChatServiceContext(
            ...
            hierarchy_actor=cast(_TAny, None),
        )
...
async def _run_actor_identity_test():
    ...
    service_context = ChatServiceContext(
        request_id="r1",
        access=ModelAccessContext(),
        hierarchy_actor=stream_hierarchy_actor,
    )
