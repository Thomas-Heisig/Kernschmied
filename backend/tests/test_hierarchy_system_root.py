...
class FakeRepository:
    ...
    async def create_node(self, data: HierarchyNodeCreate) -> Any:
        ...
        node = SimpleNamespace(
            **{
                "id": data.node_id or "generated-id",
                "is_system": False,
                "is_movable": True,
                "is_deletable": True,
            }
    ...
    async def move_node(self, node: Any, new_parent_id: str | None = None) -> None:
        node.parent_id = new_parent_id

    async def reorder_nodes(self, moves: list[tuple[str, str | None, int]]) -> None:
        return None
    ...
    async def is_descendant(
        self, node_id: str, possible_ancestor_id: str | None
    ) -> bool:
        return False
...
async def test_system_root_cannot_be_deleted():
    ...
    service = HierarchyService(
        ...
        serializer=cast(HierarchySerializer, DummySerializer()),
    )
...
async def test_reorder_immovable_nodes_forbidden():
    ...
    moves: list[tuple[str, str | None, int]] = [("n1", "p1", 0)]
