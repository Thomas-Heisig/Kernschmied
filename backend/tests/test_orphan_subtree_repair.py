...
def test_build_plan_blocks_on_children(
    ...
    async def _run():
        async with session_factory() as s:  # type: ignore
            ...
            # orphan with a child
            orphan = HierarchyNodeModel(
                id="orphan-1", type="chat", name="Conversation conversation_x"
            )
            child = HierarchyNodeModel(
                id="child-1", type="folder", name="Public", parent_id="orphan-1"
            )
            ...
            plan = await build_orphan_repair_plan(
                s,
                orphan_node_id="orphan-1",
                target_parent_id="root",
                target_chat_node_id="chat-1",
            )  # type: ignore
...
def test_plan_rejects_non_chat_orphan(
    session_factory: async_sessionmaker[AsyncSession],
):
    async def _run():
        async with session_factory() as s:  # type: ignore
...
def test_plan_rejects_non_root_orphan(
    session_factory: async_sessionmaker[AsyncSession],
):
    async def _run():
        async with session_factory() as s:  # type: ignore
            root = HierarchyNodeModel(id="root", type="user", name="Root")
            child_parent = HierarchyNodeModel(
                id="p1", type="workspace", name="WS", parent_id="root"
            )
            await s.merge(root)  # type: ignore
            ...
            orphan = HierarchyNodeModel(
                id="orphan-2",
                type="chat",
                name="Conversation conversation_y",
                parent_id="p1",
            )
            chat_node = HierarchyNodeModel(id="chat-1", type="chat", name="Chat 1")
            await s.merge(orphan)  # type: ignore
...
def test_plan_rejects_missing_target_parent(
    session_factory: async_sessionmaker[AsyncSession],
):
    async def _run():
        async with session_factory() as s:  # type: ignore
            orphan = HierarchyNodeModel(
                id="o3", type="chat", name="Conversation conversation_z"
            )
            chat_node = HierarchyNodeModel(id="chat-1", type="chat", name="Chat 1")
            await s.merge(orphan)  # type: ignore
            await s.merge(chat_node)  # type: ignore
...
def test_plan_rejects_invalid_target_chat(
    session_factory: async_sessionmaker[AsyncSession],
):
    async def _run():
        async with session_factory() as s:  # type: ignore
            orphan = HierarchyNodeModel(
                id="o4", type="chat", name="Conversation conversation_a"
            )
            await s.merge(orphan)  # type: ignore
            ...
            plan = await build_orphan_repair_plan(
                ...
                orphan_node_id="o4",
                target_parent_id="root",
                target_chat_node_id="notchat",
            )  # type: ignore
            assert plan.can_apply is False
            assert (
                "target_chat_not_type_chat" in plan.blockers
                or "target_chat_not_found" in plan.blockers
            )
...
def test_detects_cycle(session_factory: async_sessionmaker[AsyncSession]):
    async def _run():
        async with session_factory() as s:  # type: ignore
            ...
            orphan = HierarchyNodeModel(
                id="orp-c", type="chat", name="Conversation conversation_c"
            )
...
def test_apply_reassigns_chats_and_deletes_orphan(
    session_factory: async_sessionmaker[AsyncSession],
):
    async def _run():
        async with session_factory() as s:  # type: ignore
            ...
            root = HierarchyNodeModel(id="root", type="user", name="Root")
            target_chat = HierarchyNodeModel(
                id="chat-1", type="chat", name="Chat Target"
            )
            orphan = HierarchyNodeModel(
                id="orp-d", type="chat", name="Conversation conversation_d"
            )
            await s.merge(root)  # type: ignore
            await s.merge(target_chat)  # type: ignore
            await s.merge(orphan)  # type: ignore
            ...
            chat = ChatModel(
                id="conv-d", node_id="orp-d", title="Conversation conversation_d"
            )
            msg = MessageModel(
                id="m1",
            ...
            mq: Result[tuple[int]] = await s.execute(
                select(func.count())
                .select_from(MessageModel)
                .where(MessageModel.conversation_id == "conv-d")
            )
