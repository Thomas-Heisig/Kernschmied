...
async def list_chat_messages(
    request: Request,
    conversation_id: str,
    after: int | None = Query(default=None, description="Return messages with sequence_number > after"),
    ...
    if not perms.can(actor, "read", node):
        raise structured_http_error(request=request, status_code=403, code="PERMISSION_DENIED", message="Not authorized to read this conversation")
    ...
    try:
        ...
        rows = await repo.list_messages(conversation_id, limit=limit + 1, after_sequence=after)
    except Exception as exc:
        raise structured_http_error(request=request, status_code=500, code="CHAT_HISTORY_UNAVAILABLE", message="Could not load chat history", details={"error": str(exc)})
    ...
    for m in visible:
        items.append(
            ChatMessageRead(
                ...
                content=m.content,
                message_type=cast(Literal['text', 'tool_call', 'tool_result', 'reasoning', 'summary'], m.message_type),
                ui_context=getattr(m, "ui_context", None),
                sequence_number=m.sequence_number,
                status=cast(Literal['pending', 'complete', 'failed', 'cancelled'], m.status),
    ...
    return ChatHistoryResponse(schema_version="1.0", conversation_id=conversation_id, items=items, has_more=has_more, next_cursor=next_cursor)
