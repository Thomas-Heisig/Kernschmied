...
export default function UserProfilePanel() {
  const { user } = useAuth();
  const { refreshCurrentUser } = useAuth() as any;
  ...
  return (
    <div className="p-4">
      ...
      <div className="mt-6 space-x-2">
        {!editing ? (
          <>
            <button onClick={() => setEditing(true)} className="px-3 py-1 bg-sky-600 text-white rounded">Profil bearbeiten</button>
            <button className="px-3 py-1 bg-slate-200 dark:bg-slate-700 rounded">Persönliche Einstellungen</button>
          </>
        ) : (
          <>
            <button disabled={saving} onClick={handleSave} className="px-3 py-1 bg-sky-600 text-white rounded">Speichern</button>
            <button disabled={saving} onClick={() => setEditing(false)} className="px-3 py-1 bg-slate-200 dark:bg-slate-700 rounded">Abbrechen</button>
          </>
        )}
      </div>
    </div>
