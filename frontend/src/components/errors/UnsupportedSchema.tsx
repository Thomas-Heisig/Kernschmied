...
function UnsupportedSchemaComponent({
  ...
  return (
    <section
      className={[
        'animate-fade-in rounded-xl border border-warning/30 bg-warning-soft p-4 text-sm text-warning dark:border-warning/20 dark:bg-warning-soft/20 dark:text-warning',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
    ...
    >
      <div className="flex items-start gap-3">
        <div aria-hidden="true">
          <IconBadge icon={<AlertTriangle />} size="lg" variant="warning" />
        </div>

        <div className="min-w-0 flex-1">
          <h3 className="mb-2 font-semibold text-warning dark:text-warning">{title}</h3>

          <p className="mb-2 text-text-soft dark:text-gray-300">
            Für den vom Backend gelieferten Schema-Typ existiert derzeit keine registrierte
            React-Komponente.
          </p>
          ...
          <div className="rounded-lg bg-surface-muted px-3 py-2 text-text dark:bg-slate-800/60 dark:text-gray-200">
            ...
            <code className="font-mono text-warning dark:text-warning/80">{kind}</code>
          </div>

          {message && <p className="mt-3 text-text-soft dark:text-gray-300">{message}</p>}
